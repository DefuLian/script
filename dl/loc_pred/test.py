...
def get_test(loc_seq_index, batch_size, max_seq_len):
    batches = []
    ...
    num_batches = (len(loc_seq_index) + batch_size -1) / batch_size
    for batch_index in range(num_batches):
        batch_start = batch_size * batch_index
        batch_end = min(batch_size *(batch_index+1), len(loc_seq_index))
        batches.append((instances[batch_start:batch_end], target[batch_start:batch_end], seq_len[batch_start:batch_end], max_seq_len))
...
def get_batch(loc_seq, batch_size, max_seq_len):
    ...
    target=[]
    seq_len=[]
    for batch_pos in np.random.choice(range(1,len(loc_seq)-1), size=(batch_size), replace=False):
    ...
    return instances, target, seq_len, max_seq_len

def gen_dataset(loc_seq_index):
    loc_seq_str = [np.array([l for (t,l) in time_loc][:10],np.int32).tostring() for uid, time_loc in loc_seq_index]
    uid_seq = [uid for uid, time_loc in loc_seq_index]
    loc_seq_str = loc_seq_str
    dataset = tf.contrib.data.Dataset.from_tensor_slices((uid_seq, loc_seq_str))
    dataset = dataset.map(lambda u,e:(u,tf.decode_raw(tf.convert_to_tensor(e),tf.int32)))
    dataset = dataset.map(lambda u,e:{'uid':u, 'input':e[:-1], 'output':e[1:]})
