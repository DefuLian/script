...
reg_u=`paste <(grep 'reg_u' log1.txt | sed -e 's/.*reg_u=\([0-9\.]\+\).*/\1/') <(grep '^[0-9]' log1.txt |cut -f81 ) | awk 'BEGIN{p=-1}{if(p<$2) {p=$2; v=$1}}END{print v}'`
...
reg_i=`paste <(grep 'reg_i' log1.txt | sed -e 's/.*reg_i=\([0-9\.]\+\).*/\1/') <(grep '^[0-9]' log1.txt |cut -f81 ) | awk 'BEGIN{p=-1}{if(p<$2) {p=$2; v=$1}}END{print v}'`
cat log1.txt >> log.txt
rm log1.txt
...
learn_rate=`paste <(grep 'learn_rate' log1.txt | sed -e 's/.*learn_rate=\([0-9\.]\+\).*/\1/') <(grep '^[0-9]' log1.txt |cut -f81 ) | awk 'BEGIN{p=-1}{if(p<$2) {p=$2; v=$1}}END{print v}'`
cat log1.txt >> log.txt
rm log1.txt

echo "$reg_u,$reg_i,$learn_rate" > prec_recall.txt
...
mv prec_recall.txt prec_recall_bprmf.txt
